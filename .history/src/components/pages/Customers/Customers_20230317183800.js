...
function Timer() {
  ...
  useEffect(() => {
    // Fetch the timer data from an API and set the state of the component with the retrieved values
    fetch('https://example.com/timers')
  ...
  return (
    <div>
      {timers.map((timer, index) => (
        <div key={index}>
          <h2>Timer {index + 1}</h2>
          <Customers expiryTimestamp={ new Date('2023-03-18T07:00:00.000Z')} />
          {/* <Customers expiryTimestamp={timer.expiryTimestamp} /> */}
        </div>
      ))}
    </div>
  );
