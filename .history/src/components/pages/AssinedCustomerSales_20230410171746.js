...
const AssinedCustomerSales = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const addComment = async (e) => {
      e.preventDefault()
      try{
        const { data } = await axios.post(`http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:6699/api/v1/assign/comment/6433e5d6983e6798f86d12e7/6433e5d6983e6798f86d12e6`)
      }catch(e) { 
        console.log(e)
      }
    }
                  ...
                  {data?.result?.map((i, index) =>
                    i.status === "active" ? (
                      <tr key={index}>
                        ...
                        <td>
                            <i className="fa-solid fa-edit" style={{color : 'blue' , cursor : 'pointer'}} onClick={() => setShow(true)} />
                        </td>
                      </tr>
                    ) : (
                      ""
