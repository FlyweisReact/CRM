...
const AdminViewCustomer = () => {
  ...
  function AssignCustomer(props) {
    ...
    const Assign = async (e) => {
      e.preventDefault()
      try{
        const { data } = await axios.post("http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:6699/api/v1/assign" , {
          salesId : id , salesId2 ,  cuestomerId
        })
        console.log(data)
      ...
      >
        ...
        <Modal.Body>
          <Form onSubmit={Assign}>
            <Form.Group className="mb-3">
              <Form.Select aria-label="Default select example" onChange={(e) => setSalesId2(e.target.value)} > 
            ...
            <Form.Group className="mb-3">
              <Form.Select aria-label="Default select example" onChange={(e) => setSalesId2(e.target.value)} multiple > 
                <option>-- Select --</option>
            ...
            <Button type='submit' style={{backgroundColor : '#19929b' , border : '1px solid #19929b' , borderRadius : '0px'}} >Submit</Button>
          </Form>
        </Modal.Body>
      </Modal>
    );
